...
test.describe('Security auth chain (staging)', () => {
  test.describe.configure({ mode: 'serial' });

  test('login -> refresh -> me -> csrf protected mutation -> logout', async ({ page, context, request }) => {
    test.skip(!STAGING_USER.email || !STAGING_USER.password, 'Set E2E_STAGING_EMAIL and E2E_STAGING_PASSWORD to run staging security chain.');
    ...
    await page.locator('input[type="email"], input[name="email"], input[autocomplete="email"]').first().fill(STAGING_USER.email);
    await page.locator('input[type="password"], input[name="password"], input[autocomplete="current-password"]').first().fill(STAGING_USER.password);
    ...
    const apiLoginResponse = await context.request.post(`${API}/api/auth/login`, {
      data: { email: STAGING_USER.email, password: STAGING_USER.password },
